...
function installHeroScene(form) {
    const hero = form.querySelector(".ar_sortie_caisse_title > div:first-child");
    if (!hero || hero.querySelector(".ar_sc_3d_stage")) {
        return;
    }

    const stage = document.createElement("div");
    stage.className = "ar_sc_3d_stage";
    ...
    hero.addEventListener("pointermove", (event) => {
        ...
        hero.style.setProperty("--ar-sc-tilt-x", `${pointerY * 3}deg`);
        hero.style.setProperty("--ar-sc-tilt-y", `${pointerX * -4}deg`);
    });

    hero.addEventListener("pointerleave", () => {
        pointerX = 0;
        pointerY = 0;
        hero.style.setProperty("--ar-sc-tilt-x", "0deg");
        hero.style.setProperty("--ar-sc-tilt-y", "0deg");
    });
